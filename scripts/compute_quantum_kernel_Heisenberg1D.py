...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--k", type = int,
        required = True,
        help = "the max distance of 1D interactions")
    parser.add_argument(
        "--r", type = int,
        required = True,
        help = "the decay rate for long range interactions (i.e. 1/(|i-j|)^r")
    parser.add_argument(
        "--decimals", type = int,
        required = False,
        default=None,
    ...
    outpath = Path(args.outpath, f"dim_{args.dataset_dim}_ntrot_{args.n_trotter}_evo_t_{args.evo_time}_init_{args.init_state}_s_{args.init_state_seed}_k_{args.k}_r_{args.r}_dec_{args.decimals}_{args.dataset}_{args.simulation_method}.p")
    if outpath.exists():
    ...
    #FeatureMap = HamiltonianEvolutionFeatureMap(args.dataset_dim, args.n_trotter, args.evo_time, args.init_state, args.init_state_seed)
    FeatureMap= Heisenberg.Heisenberg1DFeatureMap(args.dataset_dim,args.n_trotter,args.evo_time,args.init_state,args.init_state_seed,args.k,args.r)
